...
| **3** | `transport-tcp-handoff` as a `HandoffTransport` + null-wire   | `psql` connects to the listener; the null wire closes immediately with a synthetic `FATAL` (no SQL yet). Validates the FE → BE fd-pass path end-to-end. |
| **4** | `wire-pgwire-v3` first impl — startup, simple-query (`'Q'`), SPI bridge | `psql -c 'SELECT 1'` returns `1`. `trust` auth only (no TLS, no SCRAM yet). Extended-query reports a clear "not supported" error frame. |
