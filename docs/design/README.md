...
| [deferred/planner-executor-direct-path.md](deferred/planner-executor-direct-path.md) | **deferred** | SPI-vs-direct trade-off + the survey of what's callable from `postgres.c` (none of the `exec_*` dispatchers — all `static`; the parser / analyzer / planner / Portal / CachedPlan / DestReceiver layer below them is). Three reuse strategies (tuplestore, custom DestReceiver, keep SPI) + phased adoption plan. Strategy 2 (`WireDestReceiver`) shipped for extended-query in Stage B; simple-query still uses SPI. |
| [deferred/simple-query-direct-path.md](deferred/simple-query-direct-path.md) | **deferred** | Concrete Stage-B implementation plan to route simple-query (`'Q'`) through `Portal*` + `WireDestReceiver` instead of SPI. Closes the 4–6 pp `pgbench select`/`nupdate` gap to vanilla PG by eliminating the second parse + the `SPI_tuptable` materialisation. Reuses the extended-query direct path's `WireDestReceiver`; extends `pg_transport.execution_backend` to cover `'Q'` dispatch. |
