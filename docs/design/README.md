...
| [deferred/simple-query-direct-path.md](deferred/simple-query-direct-path.md) | active | Concrete simple-query (`'Q'`) direct backend: routes through `Portal*` + `WireDestReceiver` instead of SPI, eliminating the second parse + the `SPI_tuptable` materialisation. Module layout, memory-context discipline (parse_ctx + portal context; per-stmt transients ride PG's `TopTransactionContext` via `with_xact`), and editing gotchas (FETCH_ALL == LONG_MAX, tupDesc populated by PortalStart, not PortalDefineQuery). |
