...
`HandoffHandle` whose one method `handoff(fd)` ships the socket to a
bgworker over `SCM_RIGHTS`. Inside the bgworker, a slot runner drives
...
[backend-wire.md](backend-wire.md) for the FE / BE-socket / BE-wire
implementations.
...
4. **Pre-spawned backend pool** — backends (bgworkers) that take
   ownership of handed-off kernel sockets via a slot runner
   ([backend-handoff.md](backend-handoff.md)), and run a wire layer
   ([backend-wire.md](backend-wire.md)) on the fd. SQL execution
   goes through `SPI_*`. The `pg_background`-style DSM + `shm_mq`
