...
[../future-transports.md](../future-transports.md). In v0, every
transport is a `HandoffTransport`: it owns an accept loop on a kernel
socket and hands fds to the executor. The framework does not yet ship
or support a `SessionTransport` — see
[executor-pool.md](executor-pool.md) for the deferred design.
...
| ----------------- | ------------------- | ------------------------------------------------------------- | ---------------------------------------- | ------------------------------------------------------ |
| `tcp_handoff`      | `HandoffTransport`  | TCP accept + blind `SCM_RIGHTS` fd-pass                       | (none in transport)                      | Baseline; the reference implementation. ~20 lines of code. TLS / auth / FE/BE all happen in the executor (see [handoff.md](handoff.md)). |
...
| `tls-rustls`          | [`tokio-rustls`](https://github.com/rustls/tokio-rustls) | *(none in v0)* — reserved for deferred shm_mq-path transports that terminate TLS themselves; v0 handoff transports use executor-side TLS instead |
...
/// executor pool. Returns when the shutdown token fires or the stream
/// terminates.
...
executor's PG code does it natively (see [handoff.md §3](handoff.md)).
...
deferred shm_mq path (see [executor-pool.md](executor-pool.md)).
...
the **bench harness** as a client; it has no role inside the dispatcher
or executor (which never speak libpq to themselves). Sibling crates from
...
- [api.md](api.md) — the `HandoffTransport` trait every v0 entry above
  implements.
- [handoff.md](handoff.md) — what the executor does after `handle.handoff(fd)`.
- [executor-pool.md](executor-pool.md) — *deferred* design for
  `SessionTransport` and the shm_mq general path.
- [../future-transports.md](../future-transports.md) — deferred transport
  ideas.
